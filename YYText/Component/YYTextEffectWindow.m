...
//                UIGraphicsEndImageContext();
                ...
                UIGraphicsImageRenderer *renderer = [[UIGraphicsImageRenderer alloc] initWithBounds:CGRectMake(0, 0, rect.size.width, rect.size.height)];
...
//    UIGraphicsEndImageContext();
    ...
    // Create an image renderer for the given rectangle size
    ...
    UIImage *image = [renderer imageWithActions:^(UIGraphicsImageRendererContext * _Nonnull rendererContext) {
        ...
        for (UIWindow *window in windows) {
            ...
            CGContextConcatCTM(context, YYTextCGAffineTransformGetFromViews(window, self));
            [window.layer renderInContext:context]; // render
            //[window drawViewHierarchyInRect:window.bounds afterScreenUpdates:NO]; // slower when capturing the entire window
            CGContextRestoreGState(context);
        }
    }];
